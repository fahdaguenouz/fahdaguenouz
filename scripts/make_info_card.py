...
ROWS = [
    ("host",),
    ("kv", "Role", "Assistant Developer @ Zone01"),
    ("kv", "Edu", "Digital Development '24, ISTA"),
    ("kv", "Focus", "React, Laravel, Go"),
    ("kv", "Goals", "Full-time Web/Backend Dev"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "React, HTML, CSS, Tailwind, Bootstrap"),
    ("kv", "Backend", "Laravel, Node.js, Express"),
    ("kv", "Languages", "Go, JavaScript, TypeScript, PHP"),
    ("kv", "Databases", "MySQL, MongoDB"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Level 30 Achieved at Zone01 Oujda"),
    ("bul", "Built 'Do It Yourself' platform"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">fahd@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">fahd</tspan><tspan fill="{MUTED}">@</tspan>'
